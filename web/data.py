...
def addPaperTypeDb(paperType, paperRate):
    try:
        paperTypeCollection.insert_one({"name": paperType, "rate": paperRate})
        return 1
    except:
        return 0
...
def editJobNameDb(jobName, data):
    try:
        jobNamesCollection.update_one(
            {"name": jobName}, {"$set": {"name": data["name"]}}
        )
        mycol.update_many(
            {"jobName": jobName},
            {
